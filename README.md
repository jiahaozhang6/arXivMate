...
| MiniMax | `https://api.minimaxi.com/v1` | `MiniMax-M3` | 会清理 thinking 输出。 |
...
新安装不会自动创建任何模型 Profile。保存并测试通过的 Profile 会出现在论文聊天面板的模型下拉框里，你可以按当前论文和任务手动选择。
...
- `chrome.storage.sync.settings`：模型 Profiles、语言、背景主题。
...
| MiniMax | `https://api.minimaxi.com/v1` | `MiniMax-M3` | Thinking output is cleaned when needed. |
...
New installations do not create model profiles automatically. Saved and tested profiles appear in the model selector inside the paper chat, where you can choose the model for the current paper and task.
...
- `chrome.storage.sync.settings`: model profiles, language, and appearance theme.
